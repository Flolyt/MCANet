...
#     def __init__(self, in_channels, out_channels, kernel=3, dropout=0.1, bias=False):
#         super().__init__()
#         out_ch = out_channels
#         groups = 1
#         stride = 1
...
#                                           stride=stride, padding=int(kernel/2), groups=groups, bias=bias))
...
class BasicConv2d(nn.Module):
    def __init__(self,in_channels, out_channels, kernel=3, stride=1, dropout=0.1, bias=False):
        ...
        out_ch = out_channels
        groups = 1
        self.conv = nn.Conv2d(in_channels, out_ch, kernel_size= kernel, stride=stride,
...
#         out_ch = out_channels
#         groups = 1
#         # layer=BasicConv2d(in_channels, out_channels, kernel, stride, kernel//2)
#         print(kernel, 'x', kernel, 'x', out_channels, 'x', out_channels, 'DepthWise')
#
#         self.add_module('conv', nn.Conv2d(in_channels, out_ch, kernel_size= kernel, stride=stride,
#                                           padding=kernel//2,groups=groups,bias=bias))
#         self.add_module('norm', nn.BatchNorm2d(out_ch))
#         self.add_module('relu', nn.ReLU6(True))
#
    ...
    def __init__(self, in_channels, growth_rate, grmul, n_layers, keepBase=False, residual_out=False, dwconv=False):
        ...
        for i in range(n_layers):
          ...
          if dwconv:
            ...
            layers_.append(BasicConv2d(inch, outch))
...
class HarDNet(nn.Module):
    def __init__(self, depth_wise=False, arch=85, pretrained=True, weight_path=''):
        ...
        self.base.append (
             BasicConv2d(in_channels=3, out_channels=first_ch[0], kernel=3,
                       stride=2,  bias=False) )

        # Second Layer
        self.base.append ( BasicConv2d(first_ch[0], first_ch[1],  kernel=second_kernel) )
        ...
        for i in range(blks):
            ...
            self.base.append ( BasicConv2d(ch, ch_list[i], kernel=1) )
            ch = ch_list[i]
            # print(ch)
            if downSamp[i] == 1:
              if max_pool:
...
def hardnet(arch=85,pretrained=True, **kwargs):
    if arch ==85:
        print("85 LOADED")
        model = HarDNet(arch=85)
        if pretrained:
            weights = torch.load('/home/liuyt/LPraNet-master/lib/hardnet85.pth')
